...
export const apiClient = {
  ...
  post: async (path, body, options = {}) => {
    ...
    const headers = getHeaders(isClient);
  ...
  appointments: {
    ...
    create: async (data) => {
      ...
      return await res.json(); // returns { data: {...}, payment?: {...} }
    },
    update: async (id, data) => {
  ...
  availability: {
    get: async (professionalId, serviceId, date) => {
      ...
      if (!res.ok) throw new Error('API Error');
      const json = await res.json();
      // Returns { available: ['H:i',...], all_slots: [{time, available},...] }
      return json.data;
    },
    getMonth: async (professionalId, serviceId, year, month) => {
      const res = await fetch(`${API_BASE}/availability/month?professional_id=${professionalId}&service_id=${serviceId}&year=${year}&month=${month}`, { headers: getHeaders() });
      if (!res.ok) throw new Error('API Error');
      const json = await res.json();
      return json.data; // ['Y-m-d', ...]
    }
  },
  upload: {
    image: async (file) => {
  ...
  payments: {
    getSettings: async () => {
      const res = await fetch(`${API_BASE}/settings/payment`, { headers: getHeaders(false) });
      if (!res.ok) throw new Error('API Error');
      return await res.json();
    },
    ...
    getMercadoPagoConnectUrl: async () => {
      const res = await fetch(`${API_BASE}/auth/mercadopago/connect`, { headers: getHeaders(false) });
      if (!res.ok) throw new Error('API Error');
      return await res.json(); // { url: '...' }
    },
    disconnectMercadoPago: async () => {
      const res = await fetch(`${API_BASE}/auth/mercadopago/connect`, {
        method: 'DELETE',
        headers: getHeaders(false)
      });
      if (!res.ok) throw new Error('API Error');
